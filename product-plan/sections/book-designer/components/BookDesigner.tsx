  ...
  return (
    <div className="h-screen flex flex-col bg-stone-100 dark:bg-stone-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <PageThumbnailStrip pages={book.pages} selectedPageId={selectedPage?.id ?? ''} onSelectPage={onSelectPage} onReorderPage={onReorderPage} onAddPage={onAddPage} onRemovePage={onRemovePage} />
        {selectedPage && (
          <PagePreview page={selectedPage} book={book} onDrop={(photoId) => onPlacePhoto(selectedPage.id, photoId)} onRemovePhoto={onRemovePhoto} onChangeTheme={onChangeTheme} />
        )}
        <ImageLibrary photos={photos} suggestedCoverImages={suggestedCoverImages} onPhotoClick={(photoId) => onPlacePhoto(selectedPage?.id ?? '', photoId)} onSuggestedClick={(suggestedId) => onPlacePhoto(selectedPage?.id ?? '', suggestedId)} onDragStart={handleDragStart} onUploadPhotos={onUploadPhotos} />
      </div>
    </div>
  )
}
