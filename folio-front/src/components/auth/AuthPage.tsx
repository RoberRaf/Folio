...
import type { AuthErrorCode, AuthMode, AuthProps } from './types'
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            ...
            <p className="text-stone-500 dark:text-stone-400 text-sm">
              {mode === 'signIn' ? 'Sign in to access your photo books.' : 'Start designing your first book today.'}
            </p>
          </div>

          <button type="button" onClick={handleGoogle} disabled={isLoading} className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-700 dark:text-stone-200 text-sm font-medium hover:bg-stone-50 dark:hover:bg-stone-750 hover:border-stone-300 dark:hover:border-stone-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required autoComplete={mode === 'signIn' ? 'current-password' : 'new-password'} className={inputClass} />
            </div>
            {errorMessage && (
            ...
            <button type="submit" disabled={isLoading} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-rose-500 hover:bg-rose-600 dark:bg-rose-600 dark:hover:bg-rose-500 text-white text-sm font-semibold tracking-wide transition-colors disabled:opacity-70 disabled:cursor-not-allowed mt-1">
              {isLoading ? (
                <>
          ...
          <p className="text-center text-sm text-stone-500 dark:text-stone-400 mt-6">
            ...
            <button type="button" onClick={() => handleToggle(mode === 'signIn' ? 'signUp' : 'signIn')} className="text-rose-500 dark:text-rose-400 font-semibold hover:underline underline-offset-2">
